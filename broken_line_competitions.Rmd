...
```{r fig.height=8}
display_passes("22612d6c-6e85-48ac-b4cd-4ba1f57b81ed")
```
...
```{r}
info.elim=break_line(ffs[c(0:20),],3,8,10)
info.elim
```
...
```{r}
# data360 <- StatsBombFree360Events(MatchesDF = Matches[1,1], Parallel = T)    
#     
#     events <- StatsBombFreeEvents(MatchesDF = Matches[1,1], Parallel = T)
#     
#     events <- allclean(events)
#     
#     events <- get.opposingteam(events)
...
best_passer_match=function(ffs){
  ...
  tab1 <- data.frame(name_passer = joueurs,num_elim_players=0)
  tab1$num_elim_players=as.integer(tab1$num_elim_players)
  ...
  #print("avant boucle dans line_match")
  for (k in 1:nrow(test)){
    if (test[k,"broken_line"]==TRUE){
      result = tab1[tab1[,"name_passer"]==test[k,"player.name"],"num_elim_players"]
      tab1[tab1[,"name_passer"]==test[k,"player.name"],"num_elim_players"] = result+1
    }
  }
  

  # on trie le tableau par ordre décroissant pour avoir le meilleur passeur en premier
  tab1=tab1 %>% arrange(desc(num_elim_players))
  # newdata
   return(tab1)
}
...
```
```{r}
ggplot(tableau_test) + geom_col(aes(x = num_elim_players, y = reorder(name_passer, num_elim_players)),fill = "darkblue")+
...
join_dataframe=function(data1, data2){
  ...
  for (k in 1:nrow(data2)){
    if (is.element(data2$name_passer[k],joueurs)){
      before_add = data1[data1[,"name_passer"]==data2$name_passer[k],"num_elim_players"]
      add = data2[k,"num_elim_players"]
      data1[data1[,"name_passer"]==data2$name_passer[k],"num_elim_players"] = before_add + add
...
line_breaking_all_matchs=function(Matches,begin=1,end=nrow(Matches)){
  ...
  for (i in begin:end){
    ...
    if (i == 1){
      ...
      tab1 <- data.frame(name_passer = joueurs,num_elim_players=0, number_of_matches=0)
      tab1$num_elim_players=as.integer(tab1$num_elim_players)
...
```{r}
Tab_recap = line_breaking_all_matchs(Matches,1,25)
```
#<<<<<<< HEAD
...
```
#Tab_recap_ordered <- arrange(Tab_recap, desc(num_elim_players))
#Tab_recap_ordered
#=======
#>>>>>>> e136ce1d093978a016d709a0e1ca994d487ad203
