...
best_passer_match=function(ffs){
  ...
  for (j in (1:nrow(players_pass))){
    ...
    if (players_pass[j,"length_gain"]>0){
      tab1[tab1[,"name_passer"]==players_pass[j,"player.name"],"nb_passes_forward"] = tab1[tab1[,"name_passer"]==players_pass[j,"player.name"],"nb_passes_forward"]+1
      tab1[tab1[,"name_passer"]==players_pass[j,"player.name"],"average_length_gain"] = tab1[tab1[,"name_passer"]==players_pass[j,"player.name"],"average_length_gain"]+players_pass[j,"length_gain"]
    }
  }
  
  tab1[,"average_length_gain"]=tab1[,"average_length_gain"]/tab1[,"nb_passes_forward"]
...
join_dataframe=function(data1, data2){
  ...
  for (k in 1:nrow(data2)){
    if (is.element(data2$name_passer[k],joueurs)){
      ...
      before_add = data1[data1[,"average_length_gain"]==data2$name_passer[k],"average_length_gain"]
      add = data2[k,"average_length_gain"]
      data1[data1[,"average_length_gain"]==data2$name_passer[k],"average_length_gain"] = before_add + add
      
      before_add = data1[data1[,"nb_passes_forward"]==data2$name_passer[k],"nb_passes_forward"]
      add = data2[k,"nb_passes_forward"]
      data1[data1[,"nb_passes_forward"]==data2$name_passer[k],"nb_passes_forward"] = before_add + add
...
line_breaking_all_matchs=function(Matches,begin=1,end=nrow(Matches)){
  ...
  for (i in begin:end){
    ...
    if (i == 1){
      print("on rentre dans la première boucle")
      ...
      test=line_match(ffs[,])
        ...
        #on crée un dataframe qui contient le nom de chaque joueur et le nombre de joueurs que chacun ont éliminé
        tab1 <- data.frame(name_passer = joueurs,num_elim_players_in_line=0,number_passes=0,num_eliminated_players=0,number_of_matches=0,average_length_gain=0,nb_passes_forward=0)
...
```{r}
Tab_recap = line_breaking_all_matchs(Matches,1,3)
```
